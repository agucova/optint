...
m.addConstr((quicksum(Y[p, u, f, t] for p in P) <= C[u, f]
    for u in U for f in F for t in T), name='R1')


# R2: Cambio de cama
m.addConstrs((Y[p, u, f, t - 1] - Y[p, u, f, t] <= Dif[p, u, f, t]
    for p in P for u in U for f in F for t in range(E_start[p] + 1, E_end[p] + 1)), name="R2.1")

m.addConstrs((Y[p, u, f, t] - Y[p, u, f, t - 1] <= Dif[p, u, f, t]
    for p in P for u in U for f in F for t in range(E_start[p] + 1, E_end[p] + 1)), name="R2.2")

m.addConstrs((quicksum(Dif[p, u, f, t] for u in U for f in F) == 2 * Z[p, t]
    for p in P for t in range(E_start[p] + 1, E_end[p] + 1)), name="R2.3")

m.addConstrs((Z[p, t] == 0
    for p in P for t in range(1, E_start[p] + 1)), name="R2.4")

m.addConstrs((Z[p, t] == 0
    for p in P for t in range(E_end[p] + 1, T[-1] + 1)), name="R2.5")
...
m.addConstrs((S[p, t] * Z[p, t] < UMBRAL_CRITICO 
    for p in P for t in T), name="R4")
...
m.addConstrs((quicksum(Y[p,u,f,t] for u in U for f in F) == 1 
    for t in range(E_start[p], E_end[p] + 1) for p in P), name="R6")


# R7: Si p es COVID-19 positivo, solo puede ser asignado a una unidad COVID-19.
# TODO: COV definido?
m.addConstrs(((quicksum(y[p, u, f, t] for f in F) for u in COV) == V[p] 
    for p in P for t in T), name="R7")


# R8: Antes de entrar p nno tienne asignada una cama
m.addConstrs((quicksum(Y[p, u, f, t] for u in U for f in F) == 0
    for p in P for t in range(1, E_start[p])), name="R8") # no le coloca el -1 porque no es inclusivo

# R9: Después d salir, p no tendrá asignada una cama
m.addConstrs((quicksum(Y[p, u, f, t] for u in U for f in F) == 0 
    for p in P for t in range(E_end[p] + 1, T[-1] + 1)), name="R9") # +1 para ser inclusivo


# R10: Mientras esté en el hospital, p siempre tiene asignado 1 cama
m.addConstrs((quicksum(Y[p, u, f, t] for u in U for f in F) == 0 
    for p in P for t in range(E_start[p], E_end[p] + 1)), name="R10")


# R11: p no puede ser trasladado más de X veces durante el día
# TODO: definir X
m.addConstrs((quicksum(Z[p, t] for p in P) <= x for t in range(E_start[p], E_end[p] + 1)), name="R11")
