...
pacientes = [Paciente() for _ in range(P)]
...
m.addConstrs(Y[p, u, f, t - 1] - Y[p, u, f, t] <= Dif[p, u, f, t]
    for p in P for u in U for f in F for t in range(E_start[p] + 1, E_end[p] + 1))

m.addConstrs(Y[p, u, f, t] - Y[p, u, f, t - 1] <= Dif[p, u, f, t]
    for p in P for u in U for f in F for t in range(E_start[p] + 1, E_end[p] + 1))

m.addConstrs(quicksum(Dif[p, u, f, t] for u in U for f in F) == 2 * Z[p, t]
    for p in P for t in range(E_start[p] + 1, E_end[p] + 1))

m.addConstrs(Z[p, t] == 0
    for p in P for t in range(1, E_start[p] + 1))

m.addConstrs(Z[p, t] == 0
    for p in P for t in range(E_end[p] + 1, T[-1] + 1))


# R3: Hay un máximo de cambios por hora
m.addConstrs((quicksum(Z[p] for p in P) <= A[t] for t in T))
...
m.addConstrs((alpha[p] == 1 - quicksum(...))
...
m.addConstrs(quicksum(Y[p,u,f,t] for u in U for f in F) == 1 
    for t in range(E_start[p], E_end[p] + 1) for p in P)
...
m.addConstrs(quicksum(Y[p, u, f, t] for u in U for f in F) == 0
    for p in P for t in range(1, E_start[p])) # no le coloca el -1 porque no es inclusivo

# R9: Después d salir, p no tendrá asignada una cama
m.addConstrs(quicksum(Y[p, u, f, t] for u in U for f in F) == 0 
    for p in P for t in range(E_end[p] + 1, T[-1] + 1)) # +1 para ser inclusivo


# R10: Mientras esté en el hospital, p siempre tiene asignado 1 cama
m.addConstrs(quicksum(Y[p, u, f, t] for u in U for f in F) == 0 
    for p in P for t in range(E_start[p], E_end[p] + 1))


# R11: p no puede ser trasladado más de X veces durante el día
# TODO: definir X
m.addConstrs(quicksum(Z[p, t] for p in P) <= x 
